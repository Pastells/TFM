...
def integrand_radial_period(chi, p_orbit, e_orbit):
    ...
    dt_dchi = p2 * (
        np.sqrt(((p_orbit - 2.0) ** 2 - 4.0 * e2) / (p_orbit - 4.0 - 2.0 * oneplusecoschi))
...
def compute_radial_period(p_orbit, e_orbit):
    ...
    if __name__ == "__main__":
        if error > 1.0e-12:
            print("WARNING: There may be a significant error in the Computation of the Radial Period")
...
def sco_odes_rhs(y, t, p_orbit, e_orbit):
    ...
    dchi_pdt = (
        ...
        * (
            np.sqrt((p_orbit - 6.0 - 2.0 * (e_orbit) * (np.cos(chi_p))) / ((p_orbit - 2.0) ** 2 - 4.0 * (e_orbit ** 2)))
            / (p_orbit ** 2)
        )
    )
